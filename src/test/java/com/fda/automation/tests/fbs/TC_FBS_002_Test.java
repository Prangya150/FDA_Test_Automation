...
/**
 * TC_FBS_002 - Verify that a user can place an order with 2 products (1 quantity each),
 * both fulfilled by a single 3P (FBS) seller, across FDA (Magento storefront), Mirakl
 * (marketplace operator) and Kibo Commerce (fulfillment/OMS APIs).
 *
 * Kept as a single @Test method for the same reason as TC_FBS_001_Test: BaseTest provisions one
 * WebDriver per @BeforeMethod/@AfterMethod pair, so splitting this into independent @Test methods
 * would each start a fresh, logged-out browser and lose the FDA session/cart/order state that
 * later steps depend on. The Mirakl/Kibo portion of this flow is identical to TC_FBS_001 (a single
 * order, just with 2 line items instead of 1), so those helper methods mirror TC_FBS_001_Test.
 */
public class TC_FBS_002_Test extends BaseTest {
    ...
    @Test(groups = {"regression", "fbs"},
            description = "Verify that a user can place 2 products with 1 quantity each through one 3P (FBS) seller")
    public void verifyFbsOrderWithTwoProducts() {
    ...
    private void searchAndAddFirstProductToCart() {
        ...
        FdaProductDetailsPage pdp = fdaHomePage.searchProduct(config.getFbs002Product1Sku());

        Assert.assertTrue(pdp.isDisplayed(), "PDP was not displayed for SKU " + config.getFbs002Product1Sku());
    ...
    private void searchAndAddSecondProductToCart() {
        ...
        FdaProductDetailsPage pdp = fdaHomePage.searchProduct(config.getFbs002Product2Sku());

        Assert.assertTrue(pdp.isDisplayed(), "PDP was not displayed for SKU " + config.getFbs002Product2Sku());
    ...
    private void verifyOrderHistory() {
        ...
        // Same live-confirmed status as TC_FBS_001 (fda.order.initial.status) - a freshly placed
        // order shows "Pendiente" here by default, not "Creada" as stated in the test case text.
