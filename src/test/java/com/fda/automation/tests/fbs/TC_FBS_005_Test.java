...
/**
 * TC_FBS_005 - Verify that a user can place an order with 2 products (1 quantity each), fulfilled
 ...
 *
 * CONFIRMED live on 2026-09-01: unlike the single-seller FBS tests, a two-seller checkout syncs
 * into Mirakl as *two* separate suborders (one per seller, referenced as the FDA order number plus
 * an incrementing WEB-A/WEB-B suffix - see {@link MiraklOrdersPage#findSuborderReferences}), each
 * with its own total and its own accept/documents/tracking/ship/receive lifecycle. Kibo, however,
 * stays a *single* shipment for the whole order, with one line item per seller (see
 * {@link KiboShipmentService#getAllDeliveryTypes}) - so the Mirakl-side loop and the Kibo-side
 * delivery-type check are independent and, per the sequencing below, the Mirakl fulfillment loop
 * runs to completion regardless of what the Kibo check finds. The manual test case's own 72 steps
 * describe only a single Mirakl order/shipment (matching TC_FBS_001-004), so this test adapts that
 * same sequence to loop over both suborders instead of assuming there is exactly one.
 *
 ...
 */
public class TC_FBS_005_Test extends BaseTest {
    ...
    @Test(groups = {"regression", "fbs"},
            description = "Verify that a user can place 2 products with 1 quantity each through two different 3P (FBS) sellers")
    public void verifyFbsOrderWithTwoProductsFromTwoSellers() {
    ...
    private void searchAndAddFirstProductToCart() {
        ...
        FdaProductDetailsPage pdp = fdaHomePage.searchProduct(config.getFbs005Product1Sku());

        Assert.assertTrue(pdp.isDisplayed(), "PDP was not displayed for SKU " + config.getFbs005Product1Sku());
        ...
        // Calling FDA ProductDetailsPage method to add the first product to the cart
        pdp.addToCart();
    }

    private void searchAndAddSecondProductToCart() {
        // Calling FDA HomePage method to search for the second product SKU
        FdaProductDetailsPage pdp = fdaHomePage.searchProduct(config.getFbs005Product2Sku());

        Assert.assertTrue(pdp.isDisplayed(), "PDP was not displayed for SKU " + config.getFbs005Product2Sku());
        ...
        // Calling FDA ProductDetailsPage method to add the second product to the cart
        pdp.addToCart();
    ...
    private void verifyCartContents() {
        ...
        Assert.assertEquals(cartPage.getQuantityForProduct(firstProductName), "1",
                "First product cart quantity was not 1");
        Assert.assertEquals(cartPage.getQuantityForProduct(secondProductName), "1",
                "Second product cart quantity was not 1");
    ...
    private void completeCheckoutAndPlaceOrder() {
        ...
        Assert.assertTrue(placeOrderButtonText.contains(expectedTotalDigits)
                ...
                "'Completar pago' button did not reflect the expected order total for both products. Button text: " + placeOrderButtonText);
    ...
    /**
     * CONFIRMED live on 2026-09-01 (TC_FBS_006, same risk applies here): the several minutes the
     * FDA checkout flow takes on the other tab is long enough for the Mirakl Auth0 session to
     * expire in the background, silently dropping this tab back to the login form by the time
     * Mirakl-side steps resume - which has no "Orders" menu at all, so
     * {@link MiraklOrdersPage#openOrdersMenu} just times out no matter how long it waits. Now that
     * Mirakl login happens once for the whole suite (see SuiteLoginListener) rather than fresh per
     * test, this check runs on every switch to this tab, not just once before the Mirakl steps.
     */
     ...
     * the next, per team direction: WEB-A runs end-to-end first, then WEB-B, rather than accepting
     * both suborders up front and only then looping back over them for fulfillment.
     */
    ...
    /**
     * Verifies every shipment (one per seller) has delivery type "FBS", not just the first.
     *
     * The second seller's shipment can take a few seconds to appear in Kibo after its Mirakl
     * suborder is accepted, so this polls for 2 shipments rather than asserting on a single
     * immediate read (which intermittently only sees the first shipment).
     */
    private void verifyShipmentDeliveryTypes() {
        // Calling Kibo ShipmentService to fetch the delivery type of every shipment on this order
        ...
        List<String> deliveryTypes = PollingUtils.pollUntil(
                ...
                Duration.ofSeconds(config.getKiboSyncTimeoutSeconds()),
                Duration.ofSeconds(config.getKiboSyncPollIntervalSeconds()),
                "Kibo never reported 2 shipments (one per seller) for order " + orderContext.getKiboOrderId());

        Assert.assertEquals(deliveryTypes.size(), 2,
                "Expected 2 Kibo shipments (one per seller), found: " + deliveryTypes);
